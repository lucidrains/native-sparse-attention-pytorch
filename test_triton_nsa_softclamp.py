...
from native_sparse_attention_pytorch.triton_native_sparse_attention_with_softclamp import (
...
def regular_attend(
    ...
    assert softclamp_value <= 50.

    softclamp_fn = partial(softclamp, value = softclamp_value)
...
out = regular_attend(rq, rk, rv, indices, mask, block_size = fine_block_size, sel_scale = rsel_scale, return_sliding_window_out = fused_sliding_window)
...
nsa_out = native_sparse_attend(nq, nk, nv, fine_block_size, indices, mask, sel_scale = nsel_scale, block_dk_dv_use_dot = block_dk_dv_use_dot, return_sliding_window_out = fused_sliding_window)
