...
def forward_kernel_causal_and_sparse(
    ...
    if INCLUDE_BLOCK_CAUSAL:
        ...
        for kv_block_offset_ind in range(num_kv_blocks):
            ...
            l_i_new = tl.exp(lse_i) + l_ij
            lse_i = tl.log(l_i_new)
    ...
    for off_sel_kv_block in range(NUM_SEL_KV_BLOCKS):
        ...
        for off_blocks_per_sel in range(NUM_BLOCKS_PER_SEL):
            ...
            block_qk /= SOFTCLAMP_VALUE
            block_qk = libdevice.tanh(block_qk)
            block_qk *= SOFTCLAMP_VALUE
            ...
            block_p = tl.exp(sel_qk * softmax_scale)
...
def backward_kernel_one_col_block_sparse(
    ...
    # account for softclamp backwards
    
    dtanh = dtanh.reshape(BLOCK, QUERY_HEAD_GROUPS, QUERY_EXPAND_DIM, BLOCK)
    dtanh = tl.reduce(dtanh, 2, reduce_avg)
...
class NSA(Function):
    ...
    def forward(
        ...
        return out.type(dtype), slide_out.type(dtype)

    @classmethod
    def backward(self, ctx, do, do_slide):
        device = do.device
        ...
        return dq, dk, dv, None, None, None, ret_sel_grads, None, None, None, None
...
def native_sparse_attend(
    ...
    out, sliding_out = _native_sparse_attend(
        fq, fk, fv,
        block_size,
        selected_block_indices,
        fmask,
